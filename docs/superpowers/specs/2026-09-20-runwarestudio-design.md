...
**Probe safety rules (binding).** A probe must never be a request RunWare could accept. (1) The parameter probe always carries the unknown key, which is rejected first. (2) The size probe is sent only when the parameter probe listed both `width` and `height`, always with `width: 1, height: 1` (RunWare's minimum is 128 everywhere), a valid prompt (≥ 3 characters), no `inputs`, and no other fields. (3) Models whose parameter list has no width/height get no second probe. (4) The harvest reads the account balance before and after and stops with a loud error if it changed. (5) Nothing else is ever sent by the harvest. (6) Added 2026-09-22 after the first live run: some providers validate provider-side and ACCEPT the unknown-key probe (Gemini Omni Flash, Luma Ray 3.2, Riverflow 2.0/2.5 Pro each generated and billed a result, $3.40 in total); models whose AIR starts with `google:` (Nano Banana Pro accepted it as well), `luma:` or `sourceful:` are therefore never probed (docs page only), any SDK error that can only happen after submission (a polling timeout, a provider or server error, an unknown code) is treated as an accepted probe and stops the harvest, the balance is read before and after EVERY model's probes so a run stops after at most one charge, and a model whose probe was billed is marked `constraints_json.probe.blocked` and never probed again. (7) Revised 2026-09-24 after FLUX.2 [max] (`bfl:7@1`) accepted the unknown-key probe and the run stopped at 1 of 206: an accepted probe is one charge already made, and stopping there only loses the other models' free data. The harvest therefore no longer aborts. An accepted or unknown-outcome probe blocks that model for good, puts its provider prefix on a per-run skip list (docs pages only for the rest of that run — the same validation path is likely to accept again), records the row as `error: probe … was accepted`, and continues; a balance change across one model's probes blocks that model and continues; any other failure while handling a row is that row's `error:` and the next row runs. Every such incident is listed in the closing message (`Harvested N of M … WARNING: …`, with the balance before and after), shown in the error style. The only runs that send no probes at all are still the ones that cannot verify safety in the first place: no key, a client that will not open, a balance that cannot be read before the run (docs pages only, message `STOPPED: …`). A "fully-formed request as a final check" is explicitly out of scope: RunWare validates the prompt before sizes, so no field is known to be checked last, and the search for one is what cost $1.17 on 2026-09-22.
...
**Runner.** `width`/`height` are never dropped as an ordinary field. A validation rejection whose message carries `Supported values are: '…'` is parsed into a size list and the task is retried once with the nearest listed size (smallest aspect-ratio difference, then closest area), recorded as `{"field": "width/height", "action": "corrected", "from": [w,h], "to": [w2,h2]}`; a `between A and B, in multiples of N` message snaps each side into range and to the multiple. The job service persists a correction into the model's `constraints_json.dims` (`mode: list` with the parsed list, or `rule`) with `sources.observed` set, so the next visit to the Generate page already offers the right sizes.

Added 2026-09-23 after the first full sweep of the video catalog (31 failed jobs, all validation rejections, none billed):
- *Pixels with an input image.* Several models (Kling 3 Standard, Kling 2.6 Pro, MiniMax H3 Max/Fast, HappyHorse 1.1, SkyReels V4) refuse `width`/`height` the moment `inputs.frameImages` or `inputs.referenceImages` is present — the image fixes the aspect ratio and a `resolution` preset (`"480p"`, `"720p"`, `"768p"`, `"1080p"`) picks the tier. RunWare words it as `Unsupported use of 'width' parameter … Allowed values are: … 'resolution'` (Kling, MiniMax, HappyHorse) or `Parameter 'width' and Parameter 'inputs.frameImages' cannot be used together` (SkyReels); the docs pages say "When inputs.frameImages is provided, width/height cannot be used." `runner.size_to_resolution` drops the pair and sends the preset the request was built from (`tasks.resolution_tier`: the request's preset name, or the model's own label for the posted size, snapped to the model's listed `resolution` values) when the allowed list names `resolution` or names nothing; otherwise the pair is simply dropped (H3 Fast: the frame alone sizes the clip). Recorded as `{"field": "width/height", "action": "converted_to_resolution", "to": "720p"}` or `"dropped"`, and persisted as `constraints_json.size_with_inputs` = `"resolution"` | `"none"`, which `build_video_task` honours up front for the next job. The docs harvest seeds the same key from the sentence above (`docs_pages.parse_rules` → `rules.frames_forbid_size`), and the docs' `resolution` allowed values are stored under `constraints_json.resolution.values`.
- *Enumerated values.* `Invalid value for 'duration' parameter … Supported values are: '6', '10'` (MiniMax Hailuo 02 / 2.3 Fast; 01 Live lists only `'6'`) → `runner.value_correction` moves any top-level scalar (not width/height) to the nearest listed value, once per field, ties to the smaller (cheaper) one; a named preset is compared by its number. Persisted as `constraints_json.<field>.values`, so the Generate page offers a select and the builder snaps to it.
- *Conflicting inputs.* `Parameter 'inputs.frameImages' and Parameter 'inputs.referenceImages' cannot be used together` (Grok Imagine 1.5) → `runner.conflict_field` drops the second one named (a frame is the more specific intent).
- *Too many list items.* `Invalid number of elements for 'inputs.frameImages' parameter. Frame images must contain between 0 and 1 images.` (Grok has no last-frame slot) → `runner.trim_correction` keeps the first N, persisted as `inputs.frameImages.max_items`, which hides the last-frame slot.
- *Inputs the API insists on.* A rejection that ends the job still teaches the row (`constraints.rejection_patch`, persisted by `JobRunner._persist_rejection`): `Missing required parameter: 'inputs.frameImages'` (Vidu Q2 Turbo — the docs call it optional), `'inputs' must be an object` with none sent (Wan 2.6 Flash: the frames the page offers are required), and the prose `… only works in Motion Control mode (with inputs.referenceImages and inputs.referenceVideos)` (Kling 2.6 Standard) all mark those inputs `required`, so the next submit is refused on the form before it is billed.
- *Inputs the app cannot supply.* A harvested `inputs` block that requires `audio`, `video`, `referenceVideos`, `referenceAudios` or `inputAudios` (VEED Fabric, Creatify Aurora, OmniHuman-1/1.5, P-Video-Animate, Runway Aleph, Kling 2.6 Standard once learned) makes the model not generate-capable: it is left out of the Generate dropdown, badged "needs an audio track / a reference video / an input video — not supported yet" on the Models page, and refused in pre-flight with "This model needs an audio track. VJHStudio cannot supply one yet." (`constraints.unsuppliable_input`).

Every retry above is free: RunWare rejects before it bills, and the fallback chain (`runner._validation_fallback`) tries one rule per attempt within the existing five-attempt cap.
