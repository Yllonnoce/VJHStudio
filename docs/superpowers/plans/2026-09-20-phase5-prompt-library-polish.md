...
- `promptEnhance`: `{"taskType": "promptEnhance", "taskUUID": …, "prompt": <composed truncated to 300 chars, suffix-safe via prompts.cap>, "promptMaxLength": 300, "promptVersions": <1..3>, "includeCost": True}` — **no `model` field**: verified live 2026-09-20 that the API rejects a `model` key here ("Invalid value for 'model' parameter...") while the identical task without it succeeds; the model is fixed server-side, so `model_air` on the usage row records the literal string `"promptEnhance"` instead. The SDK returns `list[dict]`, **up to one row per version** (a live reply for `promptVersions: 2` came back as a single row, so callers must tolerate fewer rows than requested), each `{text, cost}`.
...
vjhstudio/runware/tasks.py              + POLISH_SYSTEM, build_prompt_enhance (no model field), build_polish_text
